...
const CreateProperty = () => {
    ...
    const [formdata, setformdata] = useState({
        prodname: '', // Set initial value to an empty string
        description: "",
        price: 0,
        stock: 0,
        category: "",
    ...
    useEffect(() => {
        const fetchListing = async () => {
            const response = await fetch(`https://backendestate.onrender.com/listing/getuserproductlisting/${listingid}`)
            const data = await response.json();
            
            const name=data.prodname
    ...
    const updateProduct = async() => {
        try {
            const response= await fetch(`https://backendestate.onrender.com/listing/updateproduct/${listingid}`,
        {
            method:"POST",
            headers:{
        ...
        <div className='w-full  bg-gray-200 p-2'>
            ...
            <div className='max-w-[1150px] m-auto flex flex-col tabl:flex-row'>
                <div className='w-[100%] tabl:w-[50%] p-4 flex flex-col'>
                    <input required type="text" onChange={handlechange} id="prodname"  value={formdata.prodname} className='shadow-lg my-2 rounded-md h-[40px] border focus:outline-none focus:ring-[#6EB5AA] focus:border-[#6EB5AA] p-2 w-full' placeholder='Name of the Product' />
                    <textarea required id="description" onChange={handlechange} value={formdata.description} name="description" className='shadow-lg w-full my-2 rounded-md p-2 border focus:outline-none focus:ring-[#6EB5AA] focus:border-[#6EB5AA]' rows="7" placeholder='Description'></textarea>
                    <div>
                            ...
                            {categories.map((category) => (
                                <li key={category} className="w-full rounded-sm m-2">
                                    <div className="flex items-center">
                                        <input onChange={handleCategoryChange} id={category} type="radio" name="category" value={category} checked={formdata.category === category} className="w-7 h-7 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500" />
                    ...
                    <div className='flex flex-col mobxl:flex-row'>
                        <div>
                            <input onChange={handlechange} value={formdata.price} required id='price' className='shadow-lg my-2 mx-1 rounded-md h-[45px] mobxl:w-[70%] border focus:outline-none focus:ring-[#6EB5AA] focus:border-[#6EB5AA] p-2' type="number" />
                            <br />
                            <span className='text-gray-700 ml-1'>Price(₹)</span>
                        </div>
                        <div>
                            <input onChange={handlechange} value={formdata.stock} required id='stock' className="shadow-lg my-2 mx-1 rounded-md h-[45px] mobxl:w-[80%] border focus:outline-none focus:ring-[#6EB5AA] focus:border-[#6EB5AA] p-2" type="number" />
                            <br />
                ...
                <div className='w-[100%] tabl:w-[50%] p-4'>
                    ...
                    <button onClick={updateProduct}  className='w-full h-[45px] rounded-md bg-[#6EB5AA] text-white font-semibold my-2 p-2'>Update Product</button>
